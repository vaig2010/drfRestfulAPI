...
class ReferralCodeViewSetTest(APITestCase):
    ...
    def test_list_referral_codes(self):
        ...
        ReferralCode.objects.create(user=self.user, code='test_code', expiration_date=expiration_date)
    ...
    def test_delete_own_referral_code(self):
        expiration_date = timezone.now() + timezone.timedelta(days=30)
        referral_code = ReferralCode.objects.create(user=self.user, code='test_code', expiration_date=expiration_date)
    ...
    def test_delete_other_user_referral_code(self):
        ...
        expiration_date = timezone.now() + timezone.timedelta(days=30)
        referral_code = ReferralCode.objects.create(user=self.other_user, code='test_code', expiration_date=expiration_date)
        url_detail = reverse('referral_codes-detail', args=[referral_code.id])
        response = self.client.delete(url_detail)
